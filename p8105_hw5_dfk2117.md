...
The raw dataset contains 52179 rows and 12 variables: `uid` which is an
identifier variable with the first 3 letters of the city and what I
presume is a numeric identifier, the `reported_date` variable just gives
the data of the homicide, `victim_last`, `victim_first`, `victim_race`,
`victim_age` and `victim_sex` gives demographic information on the
victim. `city`, `state`, `lat` and `lon` gives the location of the
homicide. Finally, `disposition` shows whether or not the case has been
closed.
...
This code chunk creates a `city_state` variable and counts the number of
unsolved cases and the number of solved cases and puts them into a
dataframe `disposition_df`. The `city_state` variable is made using the
`mutate` function along with the `paste` function. Further, `summarise`
is used to calculate the total number of rows, `n_homicides`, and the
total number of rows with disposition “Closed without arrest” and
“Open/No arrest”, `n_unsolved`.
...
The above chunk first filters out the `city_state` “Baltimore, MD”, then
performs a prop test, uses `broom::tidy()` to make a tibble out of the
prop test, then selects the estimated proportion, the lower bound of the
confidence interval and the upper bound of the confidence interval.

For this next part, I had trouble with my code, so I revised using the
solution code.
...
This code chunk performs a prop test for each `city_state`. It selects
only the `city_state`, `estimate`, `conf.low` and `conf.high` variables.
...
This last code chunk creates the deisred `geom_point` plot which plots
the `city_state` against the `estimate` value for each state. It them
uses `geom_errorbar` to put error bars corresponding to the upper and
...
![](p8105_hw5_dfk2117_files/figure-gfm/unnamed-chunk-9-1.png)<!-- --> In
...
![](p8105_hw5_dfk2117_files/figure-gfm/unnamed-chunk-11-1.png)<!-- -->
...
![](p8105_hw5_dfk2117_files/figure-gfm/unnamed-chunk-12-1.png)<!-- -->
